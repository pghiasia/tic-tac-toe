...
$ofctl add-flow s1 \
    ip,dl_dst=B0:B0:B0:B0:B0:B0,actions=mod_dl_src:0A:00:0A:FE:00:02,mod_dl_dst:0A:00:04:01:00:01,output=2

# Rule for packets from Bob back to Alice
$ofctl add-flow s1 \
    ip,dl_dst=AA:AA:AA:AA:AA:AA,actions=mod_dl_src:0A:00:01:01:00:01,mod_dl_dst:AA:AA:AA:AA:AA:AA,output=1

# Rule for packets from Alice to gateway (for forwarding to Bob via r1)
$ofctl add-flow s1 \
    ip,dl_dst=0A:00:01:01:00:01,actions=mod_dl_src:0A:00:0A:FE:00:02,mod_dl_dst:0A:00:04:01:00:01,output=2
...
$ofctl add-flow s2 \
    ip,dl_dst=B0:B0:B0:B0:B0:B0,actions=mod_dl_src:0A:00:02:01:00:01,mod_dl_dst:B0:B0:B0:B0:B0:B0,output=1

# Rule for packets from Bob to gateway (for forwarding to Alice via r1)
$ofctl add-flow s2 \
    ip,dl_dst=0A:00:02:01:00:01,actions=mod_dl_src:0A:00:0D:01:00:03,mod_dl_dst:0A:00:0E:FE:00:02,output=3

# Carol <--> David
# Layer-2 switch s2: match only on dl_dst (for IP packets)
# Rule for packets from David to gateway (for forwarding to Carol via r2)
$ofctl add-flow s2 \
    ip,dl_dst=0A:00:0B:FE:00:02,actions=mod_dl_src:0A:00:0C:FE:00:04,mod_dl_dst:0A:00:05:01:00:01,output=4

# Rule for packets from r2 to David
$ofctl add-flow s2 \
    ip,dl_dst=D0:D0:D0:D0:D0:D0,actions=mod_dl_src:0A:00:0B:FE:00:02,mod_dl_dst:D0:D0:D0:D0:D0:D0,output=2
...
$ofctl add-flow s3 \
    ip,dl_dst=CC:CC:CC:CC:CC:CC,actions=mod_dl_src:0A:00:03:01:00:01,mod_dl_dst:CC:CC:CC:CC:CC:CC,output=1

# Rule for packets from Carol to gateway (for forwarding to David via r2)
$ofctl add-flow s3 \
    ip,dl_dst=0A:00:03:01:00:01,actions=mod_dl_src:0A:00:0D:FE:00:02,mod_dl_dst:0A:00:05:01:00:01,output=2
