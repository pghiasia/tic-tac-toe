...
# Rule for packets from r1 back to Alice (r1 sets dl_dst to s1-eth2 MAC)
$ofctl add-flow s1 \
...
# Layer-2 switch s2: match only on dl_dst
# Rule for packets from r1 to Bob (r1 sets dl_dst to s2-eth3 MAC)
$ofctl add-flow s2 \
...
# Rule for packets from r2 to David (r2 sets dl_dst to s2-eth4 MAC)
$ofctl add-flow s2 \
...
# Layer-2 switch s3: match only on dl_dst
...
# Rule for packets from r2 to Carol (r2 sets dl_dst to s3-eth2 MAC)
$ofctl add-flow s3 \
